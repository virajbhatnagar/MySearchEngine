...
import com.myProject.searchengine.config.WikiPageParsingConstants;
import com.myProject.searchengine.services.PageParser;
import com.myProject.searchengine.services.Stemmer;
import com.myProject.searchengine.services.Main;
import com.myProject.searchengine.services.PageParser.Fields;

import java.io.*;
import java.util.*;
import java.util.Map.Entry;

public class SearchMain extends Browser {
	...
	public static void main1(String query) throws Exception  {
		// TODO Auto-generated method stub

		//long start = System.currentTimeMillis();
		final String indexFolder = "C:\\Users\\windows\\Desktop\\Search Project\\SearchEngine-master\\src\\main\\output";
		...
		for(int i=0;  i < numOfQueries;i++){
			...
			if(queryWords.size() == 1){
				...
				Browser.textArea.setText("\n\n\n\t\t               NO MATCH FOUND!!!\n\n\t\t              TRY AGAIN USING DIFFERENT KEYWORD");
			...
			for(int index=andDocDetails.size()-1;index>=1;index--){
				if(shownResults.size() >= WikiPageParsingConstants.MIN_RESULTSET)
					break;
				l = displayResults(queryWords,andDocDetails.get(index),index+1,shownResults);
	...
	private static List<Integer> displayResults(List<QueryWord> queryWords,
		...
		return showingDocIds;
	}
	public static void displayTitles(List<Integer> docIds) throws IOException{
		String title, res="";int count =0;
		for(Integer docId:docIds){
			title=getTitle(docId);
			if(title == null)
				continue;
			res = res +"\t\t       " + ++count + " TITLE :" +  title+"\n" ;
		}
			Browser.textArea.setText( res);
	}
	...
	static String getTitle(Integer word) throws IOException{
